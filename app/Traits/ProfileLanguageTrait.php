        ...
        if (isset($user) && count($user->profileLanguages)):
            foreach ($user->profileLanguages as $language):
                ...
                $html .= '<tr id="language_' . $language->id . '">
						...
						<td><a href="javascript:void(0);" onclick="showProfileLanguageEditModal(' . $language->id . ');" class="text text-warning">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="delete_profile_language(' . $language->id . ');" class="text text-danger">' . __('Delete') . '</a></td>
								</tr>';
            endforeach;
        endif;
