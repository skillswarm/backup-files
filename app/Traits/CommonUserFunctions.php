...
trait CommonUserFunctions
{
    ...
    private function deleteMentorImage($id)
    {
        try {
            $mentor = Mentor::findOrFail($id);
            $image = $mentor->image;
            if (!empty($image)) {
                File::delete(ImgUploader::real_public_path() . 'mentor_images/thumb/' . $image);
                File::delete(ImgUploader::real_public_path() . 'mentor_images/mid/' . $image);
                File::delete(ImgUploader::real_public_path() . 'mentor_images/' . $image);
            }
            return 'ok';
        } catch (ModelNotFoundException $e) {
            return 'notok';
    ...
    private function deleteSkillProviderImage($id)
    {
        try {
            $SkillProvider= SkillProvider::findOrFail($id);
            $image = $SkillProvider->logo;
            if (!empty($image)) {
                File::delete(ImgUploader::real_public_path() . 'skill_provider_images/thumb/' . $image);
                File::delete(ImgUploader::real_public_path() . 'skill_provider_images/mid/' . $image);
                File::delete(ImgUploader::real_public_path() . 'skill_provider_images/' . $image);
            }
            return 'ok';
        } catch (ModelNotFoundException $e) {
            return 'notok';
