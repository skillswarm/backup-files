        ...
        if (isset($user) && count($user->profileCvs)):
            ...
            foreach ($user->profileCvs as $cv):
                ...
                $html .= '<tr id="cv_' . $cv->id . '">
									...
									<td><a href="javascript:void(0);" onclick="showProfileCvEditModal(' . $cv->id . ');" class="text text-warning">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="delete_profile_cv(' . $cv->id . ');" class="text text-danger">' . __('Delete') . '</a></td>
								</tr>';
            endforeach;
        endif;
