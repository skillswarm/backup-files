        ...
        if (isset($user) && count($user->profileSkills)):
            foreach ($user->profileSkills as $skill):
                ...
                $html .= '<tr id="skill_' . $skill->id . '">
						...
						<td><a href="javascript:void(0);" onclick="showProfileSkillEditModal(' . $skill->id . ');" class="text text-warning">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="delete_profile_skill(' . $skill->id . ');" class="text text-danger">' . __('Delete') . '</a></td>
								</tr>';
            endforeach;
        endif;
