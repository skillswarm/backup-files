...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'first_name','middle_name','last_name','name', 'email', 'password',
    ];
    ...
    public function getCurrentProgress($user_id)
    {
        ...
        if(isset($user->name) && isset($user->email) && isset($user->gender) && isset($user->country_id) && isset($user->state_id) && isset($user->city_id) && isset($user->job_experience_id))
        {
          $status += 10;
        }
        if(($this->profileSummary()->count() !=0 )){
          $status += 10;
        }
        if(($this->profileCvs()->count() !=0 )){
          $status += 10;
        }
        if(($this->profileProjects()->count() !=0 )){
          $status += 10;
        }
        if(($this->profileExperience()->count() !=0 )){
          $status += 10;
        }
        if(($this->profileEducation()->count() !=0 )){
          $status += 10;
        }
        if(($this->profileSkills()->count() !=0 )){
          $status += 10;
        }
        if(($this->profileLanguages()->count() !=0 )){
          $status += 10;
        }
        if($user->verified == 1){
          $status += 10;
