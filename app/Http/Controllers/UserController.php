...
class UserController extends Controller
{
    ...
    public function updateMyProfile(UserFrontFormRequest $request)
    {
        ...
        $log = new Timeline();
        $log->user_id = $user->id;
        $log->activity_type = 'PROFILE UPDATED';
        $log->comment = 'Profile updated successfully.';
        $log->save();
    ...
    {
        Recommendation::where('candidate_id', Auth::user()->id)->update(['is_seen' => 1]);
        $user = User::findOrFail(Auth::user()->id);
        $recommendations = Recommendation::where('candidate_id', '=', $user->id)
                ->orderBy('created_at', 'desc')
                ->get();
        return view('user.applicant_recommendations')
                        ->with('user', $user)
    ...
    public function saveReply(Request $request)
    {
         ...
         $existing = MentorMessage :: where('mentor_id','=',$mentor_id)
                         ->where('candidate_id','=',Auth::user()->id)
                         ->where('recommendation_id','=',$recommendation_id)->exists();

         if($existing == 1 ){
           $mentorMessage = MentorMessage :: where('mentor_id','=',$mentor_id)
                           ->where('candidate_id','=',Auth::user()->id)
                           ->where('recommendation_id','=',$recommendation_id)
                           ->update(['message' => $reply]);
            return response()->json(array('msg'=> 'Reply Updated successfully!'));
         }else{
           $mentorMessage = new MentorMessage();
           $mentorMessage->mentor_id = $mentor_id;
           $mentorMessage->candidate_id = Auth::user()->id;
           $mentorMessage->recommendation_id = $recommendation_id;
           $mentorMessage->message = $reply;
           $mentorMessage->save();
           return response()->json(array('msg'=> 'Reply sent successfully!'));
         }
    }
    ...
    {
        $user = User::findOrFail(Auth::user()->id);
        $logs = Timeline::where('user_id',$user->id)->get();
