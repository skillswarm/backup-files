...
class UserController extends Controller
{
    ...
    public function storeUser(UserFormRequest $request)
    {
        ...
        $this->updateUserFullTextSearch($user);
            ...
            $log = new Timeline();
            $log->user_id = $user->id;
            $log->activity_type = 'PROFILE CREATED';
            $log->comment = 'Profile is created by Admin.';
            $log->save();
    ...
    public function updateUser($id, UserFormRequest $request)
    {
        ...
        $this->updateUserFullTextSearch($user);
            ...
            /*  Activity Log */
          if($request->input('verified') == 2){
            $log = new Timeline();
            $log->user_id = $user->id;
            $log->activity_type = 'PROFILE VERIFIED';
            $log->comment = 'Profile is verified by Admin.';
            $log->save();
          }else{
            $log = new Timeline();
            $log->user_id = $user->id;
            $log->activity_type = 'PROFILE UPDATED';
            $log->comment = 'Profile is updated by Admin.';
            $log->save();
          }
