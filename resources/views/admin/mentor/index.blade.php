...
<div class="page-content-wrapper">
    ...
    <div class="page-content">
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h3 class="page-title">Manage Mentors  </h3>
        <!-- END PAGE TITLE-->
        <!-- END PAGE HEADER-->
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        <div class="caption"> <i class="icon-settings font-dark"></i> <span class="caption-subject font-dark sbold uppercase">Mentors</span> </div>
                        <div class="actions"> <a href="{{ route('create.mentor') }}" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Mentor</a> </div>
                    </div>
...
<script>
    ...
    function delete_mentor(id) {
        if (confirm('Are you sure! you want to delete ?')) {
            $.post("{{ route('delete.mentor') }}", {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
    ...
    function make_active(id) {
        $.post("{{ route('make.active.mentor') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
    ...
    function make_not_active(id) {
        $.post("{{ route('make.not.active.mentor') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
    ...
    function make_verified(id) {
        $.post("{{ route('make.verified.mentor') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
    ...
    function make_not_verified(id) {
        $.post("{{ route('make.not.verified.mentor') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
