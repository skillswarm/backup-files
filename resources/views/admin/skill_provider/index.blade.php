    ...
    <div class="page-content">
        ...
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li> <span>Skill Providers</span> </li>
            </ul>
        </div>
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h3 class="page-title">Manage Skill Providers  </h3>
        <!-- END PAGE TITLE-->
        <!-- END PAGE HEADER-->
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        <div class="caption"> <i class="icon-settings font-dark"></i> <span class="caption-subject font-dark sbold uppercase">Skill Providers</span> </div>
                        <div class="actions"> <a href="{{ route('create.skill-provider') }}" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Skill Provider</a> </div>
                    </div>
                                ...
                                <table class="table table-striped table-bordered table-hover"  id="user_datatable_ajax">
                                    <thead>
                                        <tr role="row" class="filter">
                                            <td><input type="text" class="form-control" name="id" id="id" autocomplete="off" placeholder="Id"></td>
...
<script>
    $(function () {
        var oTable = $('#user_datatable_ajax').DataTable({
             ...
             */
            ajax: {
                url: '{!! route('fetch.data.skill_providers') !!}',
                data: function (d) {
    ...
    function delete_skill_provider(id) {
        if (confirm('Are you sure! you want to delete ?')) {
            $.post("{{ route('delete.skill-provider') }}", {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
    ...
    function make_active(id) {
        $.post("{{ route('make.active.skill-provider') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
    ...
    function make_not_active(id) {
        $.post("{{ route('make.not.active.skill-provider') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
    ...
    function make_verified(id) {
        $.post("{{ route('make.verified.skill-provider') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
    ...
    function make_not_verified(id) {
        $.post("{{ route('make.not.verified.skill-provider') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
